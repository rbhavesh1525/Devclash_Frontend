...
const ModelTest = () => {
  ...
  const renderFormattedFeedback = (text) => {
    ...
    return lines.map((line, i) => {
      ...
      // Headings (with emojis) - make them bolder and spaced
      if (
        trimmed.startsWith("✅") ||
        trimmed.startsWith("❌") ||
        trimmed.startsWith("📌") ||
        ...
        return (
          <div
            key={i}
            className="mt-6 mb-2 text-lg font-bold text-gray-900 border-b pb-1"
          >
            {trimmed}
          </div>
        );
      ...
      // List items (suggestions or corrections)
      if (trimmed.startsWith("*")) {
        // Check if it's **bold** list item
        const parts = trimmed.split(/(\*\*.*?\*\*)/g);
  
        return (
          <div key={i} className="ml-4 pl-2 mb-1 list-disc text-sm text-gray-800">
      ...
      // Topics array as inline code
      if (trimmed.startsWith("[")) {
        return (
          <div
            key={i}
            className="ml-2 mb-3 p-2 bg-gray-100 rounded text-sm text-gray-800 font-mono"
          >
            {trimmed}
          </div>
        );
      ...
      return (
        <p key={i} className="mb-3 text-sm text-gray-700 leading-relaxed">
          {trimmed}
        </p>
      );
    });
